...
// 取得目前語言的網路錯誤提示文字（在 Vue 元件外部呼叫，無法用 this.$t，改直接讀語言包）
// 語言判斷邏輯與 main.js 開機時一致：優先讀後端同步下來的 user_fallback_language，
// 其次退回系統語言，找不到對應翻譯則退回英文
function getNetworkErrorMsg() {
	let lang = 'en'
	try {
		lang = uni.getStorageSync('user_fallback_language') || uni.getLocale() || 'en'
	} catch (e) {}
	const table = locales[lang] || locales.en
	return (table && table['main.network_error']) || 'Network error. Please try again.'
}
