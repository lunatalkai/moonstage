...
describe('政策表', () => {
  it('酒館 → 訊息層前綴；MMD → 原樣；沒宣告／不認得 → 當 MMD', () => {
    expect(stylePolicyFor('tavern')).toEqual({ scope: MESSAGE_SCOPE, fencedDocument: 'iframe', theme: 'host' })
    expect(stylePolicyFor('mmd')).toEqual({ scope: null, fencedDocument: 'inline', theme: 'dark' })
    expect(stylePolicyFor(undefined)).toEqual({ scope: null, fencedDocument: 'inline', theme: 'dark' })
    expect(stylePolicyFor('')).toEqual({ scope: null, fencedDocument: 'inline', theme: 'dark' })
    expect(stylePolicyFor('whatever')).toEqual({ scope: null, fencedDocument: 'inline', theme: 'dark' })
  })
  it('伺服器欄位的別名也走同一張表', () => {
    expect(stylePolicyFor(' SillyTavern ')).toEqual({ scope: MESSAGE_SCOPE, fencedDocument: 'iframe', theme: 'host' })
  })
})
