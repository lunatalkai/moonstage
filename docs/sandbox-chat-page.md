...
- **更早的歷史**：捲到頂附近殼向宿主要下一頁，整頁以 `message.new` + `before` 插在最前面（先當空殼），
  捲動位置由殼補償、畫面不動；插進來的每則發 `message:new`，定稿的 AI 由宿主接著補一次 `message:done`（跟即時訊息同一條路）。
  殼問過一次後宿主一定回一次 `history` 現況（拒絕也回），殼才不會卡在「正在載」。
...
- `canvas.vue`：`applyAuthorAsset` 看到 `pageMode === 'sandbox'` → 進沙箱模式：**只有訊息列表**換成 `<iframe>`；
  ...
  **宿主層面板的皮膚**：模型設定開著的期間，宿主把作者的樣式表（規則裡的 `<style>`）與殼回報的文件狀態
  （`docstate`：作者腳本在 html／body 上記的 class 與 data-*，例如日夜開關）套到宿主頁，只套樣式、不跑腳本；
  關掉就撤掉並還原。作者樣式本來就能改整個舊頁，這裡沒有比舊頁更多的權限。
